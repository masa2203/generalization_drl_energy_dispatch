...
VERBOSE = 0
LOGGER_TYPE = ["csv"]
SAVE_PATH = os.path.join(src_dir, 'log', ENV_KWARGS['env_name'], 'dppo', 'run', input('Save in folder: ')) \
    if CREATE_LOG else None

# ACTIONS
discretization_params = {
    'gt_specs': [{'start': -1, 'stop': 1, 'num': 2}],
    'p2g_specs': [{'start': -1, 'stop': 1, 'num': 2}],
    'bes_specs': [{'start': -1, 'stop': 1, 'num': 3}],
}
...
EXP_PARAMS = {
    ...
    'p2g_soc_penalty': (500, 0.01),  # Tuple (Penalty weight, upper SOC limit)
    'p2g_inactivity_penalty': {'penalty': 2000, 'alpha': 0.02, 'threshold': 0.8},
...
RL_PARAMS: dict[str, Any] = {
    ...
    'device': 'cpu',
    'learning_rate': 9.08e-5,  # Default: 3e-4
    'use_lr_sched': True,
    'n_steps': 2048,  # Default: 2048
    'batch_size': 64,  # Default: 64
    'n_epochs': 10,  # Default: 10
    # 'gamma': 1-0.003512209278341583,  # Default: 0.99
    'gamma': 0.996474037,  # Default: 0.99
    'gae_lambda': 0.916325566,  # Default: 0.95
    ...
    'max_grad_norm': 1.695110812,  # Default: 0.5
