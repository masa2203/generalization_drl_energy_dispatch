...
class AdvancedGTModel(BaseGTModel):
    ...
    :param min_action: A float that represents the minimum possible action, used if GT must be kept on. Default is 0.0.
    ...
    def __init__(self,
        ...
        :param use_ramp_rates: Boolean (False is default) if ramping constraints are ignored,
                                otherwise tuple with (max ramp up, max ramp down) in MW/min
        :param min_action: A float that represents the minimum possible action, used if GT must be kept on.
                            Default is 0.0.
        :param resolution_h: A float that represents the resolution in hours for the simulation steps. Default is 1.0.
        :param tracking: An optional boolean that specifies whether to enable tracking.
        """
...
class A35(AdvancedGTModel):
    ...
    :param generator_efficiency: A float that represents the generator efficiency (fraction).
    :param gearbox_efficiency: A float that represents the gearbox efficiency (fraction).
    :param init_strategy: A string that specifies the initialization strategy. Valid values are 'zero' or 'random'.
    ...
    :param life_factor: A dictionary that represents the life factor.
    :param model_replace: A string that represents the DLL replacement.
    :param use_piecewise_approx: A boolean that indicates whether a piecewise approximation of the GT is used.
    :param use_ramp_rates: Boolean (False is default) if ramping constraints are ignored,
                        otherwise tuple with (max ramp up, max ramp down) in MW/min
    :param min_action: A float that represents the minimum possible action, used if GT must be kept on. Default is 0.0.
    ...
    # Set the GT-specific upper bounds (for random initialization & stats)
    max_power = 35  # in MW
    ...
    def step(
            ...
            action,
            idx: int,
            avail_fuel: Optional[float] = None,  # in lbs, None = no fuel limitations
            gas_price: Optional[float] = None,
            carbon_tax_rate: Optional[float] = None,
        ...
        :param avail_fuel: A float that represents the quantity of fuel available (in lbs). Default is None.
        :param gas_price: A float that represents the price for natural gas (in $/GJ). Default is None.
            If None, initial value is used.
        :param carbon_tax_rate: A float that represents the carbon tax rate in $/m3 (fuel) or $/MWh (energy).
            Default is None. If None, initial value is used.
        :param amb_temp_k: A float that represents the ambient temperature in Kelvin.
        :param amb_pressure_pa: A float that represents the ambient pressure in Pascal.
        :param rh_pct: A float that represents the relative humidity in percent.
        ...
        # Update gas price and carbon tax if values are provided
        if gas_price is not None:
            self.price_natural_gas = gas_price

        if carbon_tax_rate is not None:
            self.carbon_tax['rate'] = carbon_tax_rate

        # Prevents GT from running low power levels (likely to cause simulation errors)
        if action < self.operating_threshold:
            action = 0

        # Prevents GT from running below min threshold (used for example where heat is needed from GT)
        if action < self.min_action:
            action = self.min_action

        # MODEL RETURN
        if avail_fuel is not None:
            # Check max. action possible with available fuel and use as upper bound
            max_action = self.fuel_to_action(fuelflow=avail_fuel / self.resolution_h)
            action = min(action, max_action)

        # Use the piecewise linear approximation
    ...
    def piecewise_linear_a35(self, action):
        """
        ...
        """
        gt_power = action * self.max_power
    ...
    def fuel_to_action(self, fuelflow):
        """
        Returns the corresponding action for a given fuel flow, within the bounds of the piecewise linear function.

        :param fuelflow: Fuel flow in pph.
        :return: Corresponding action value, upper bounded by 1.
        """
        # Define the breakpoints of the piecewise linear function
        breakpoint_fuelflow = 360 * 1 + 2200  # Calculate fuelflow at the breakpoint (action=1)

        if fuelflow < breakpoint_fuelflow:
            gt_power = (fuelflow - 1550) / 700  # Use the first linear segment's inverse
        else:
            gt_power = (fuelflow - 2200) / 360  # Use the second linear segment's inverse

        # Convert gt_power to action.
        action = gt_power / self.max_power
