...
class P2GBaseEnv(BaseEnv):
    """
    A parent class for environments operating a P2G system.
    ...
    def __init__(self,
        ...
        # Storage
        self.p2g_dict = p2g
        self.p2g = self._init_p2g()
        self.p2g_power = None  # required for ActionCorrectionPenalty wrapper
    ...
    def reset(self, seed=None, options=None):
        ...
        # Reset storage and get first observation
        self.p2g.reset(options={'tracking': self.tracking})
        obs, done = self._get_obs()
        return obs, self.return_episode_info()
    ...
    def _init_p2g(self):
        """
        Initializes the P2G model of the environment.

        :return: A P2G object that represents the P2G model of the environment.
        """
        # Simple storage without degradation
        p2g = P2G(**self.p2g_dict,
                  resolution_h=self.resolution_h,
                  tracking=self.tracking)
        return p2g
    ...
    def _get_episode_info(self):
        ...
        """
        self.tracked_vars.update({
            'p2g_socs': self.p2g.socs,
            'p2g_power_flows': self.p2g.power_flows,
            'p2g_costs': self.p2g.p2g_costs,
            'p2g_lost_profits': self.p2g.lost_profits,
        })
        return self.tracked_vars
...
class GasTurbineBatteryP2GBaseEnv(GasTurbineBaseEnv, BatteryBaseEnv, P2GBaseEnv):
    """
    A parent class for environments operating one or more gas turbines, a battery, and a P2G system.
    ...
    def _get_episode_info(self):
        """
        Aggregates tracked variables from all components (GTs, BES, and P2G) of the environment.

        :return: A dictionary containing the aggregated tracking variables from all components of the environment.
        """
        combined_tracked_vars = {}

        gt_info = GasTurbineBaseEnv._get_episode_info(self)
        combined_tracked_vars.update(gt_info)

        bes_info = BatteryBaseEnv._get_episode_info(self)
        combined_tracked_vars.update(bes_info)

        p2g_info = P2GBaseEnv._get_episode_info(self)
        combined_tracked_vars.update(p2g_info)

        return combined_tracked_vars
